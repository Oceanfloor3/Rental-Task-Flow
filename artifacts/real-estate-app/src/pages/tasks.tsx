...
export default function Tasks() {
  ...
  if (!user?.isActive) {
    return (
      ...
      >
        <div className="w-20 h-20 rounded-full bg-[#1a2f50] flex items-center justify-center mb-5">
        ...
        <div className="w-full max-w-xs space-y-3 mb-6">
          <div className="flex items-start gap-3 bg-[#111e35] rounded-2xl p-4 text-left">
  ...
  if (isLoadingTasks || isLoadingSummary) {
    return (
      <div className="p-4 space-y-4 pb-28">
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-white/10 animate-pulse">
          <div className="h-5 bg-[#111e35] rounded w-40 mb-4" />
          <div className="h-2.5 bg-[#111e35] rounded-full w-full mb-3" />
          <div className="h-10 bg-[#111e35] rounded-xl" />
        </div>
        {[1, 2, 3, 4].map(i => (
          <div key={i} className="bg-white rounded-xl shadow-sm border border-white/10 p-4 flex gap-4 animate-pulse">
            <div className="w-24 h-24 bg-[#111e35] rounded-lg shrink-0" />
            <div className="flex-1 space-y-2 py-1">
              <div className="h-4 bg-[#111e35] rounded w-3/4" />
              <div className="h-3 bg-[#111e35] rounded w-1/2" />
              <div className="h-3 bg-[#111e35] rounded w-1/3" />
              <div className="flex justify-between items-center mt-3">
                <div className="h-4 bg-[#111e35] rounded w-20" />
                <div className="h-8 bg-[#111e35] rounded-lg w-20" />
              </div>
            </div>
          </div>
        ))}
  ...
  if (isErrorTasks) {
    return (
      <div className="p-4 flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <p className="text-white/40 text-sm mt-1">Please try logging in again</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      {/* Summary card */}
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-white/10">
        <div className="flex items-center justify-between mb-1">
          <div>
            <h1 className="text-lg font-bold text-slate-800">Today's Rental Quests</h1>
            {total > 0 && (
              <p className="text-xs text-white/40 mt-0.5">{total} quest{total === 1 ? "" : "s"} assigned to your level</p>
            )}
          </div>
          <span className="text-xs font-bold text-[#9a7a18] bg-[#111e35] px-2.5 py-1 rounded-full">
            {completed}/{total} done
          </span>
        </div>

        <div className="h-2 w-full bg-[#111e35] rounded-full overflow-hidden my-3">
          <motion.div
            initial={{ width: 0 }}
        ...
        <div className="grid grid-cols-3 gap-2 mt-3">
          ...
          <div className="bg-[#111e35] rounded-xl p-3 flex items-center gap-2">
            ...
            {taskList.map((task, idx) => (
              <motion.div
                ...
                className={`bg-white rounded-xl shadow-sm border overflow-hidden relative ${
                  task.status === "completed" ? "border-green-100" : "border-white/10"
                }`}
              >
                ...
                <div className="flex p-3 gap-3">
                  <div className="w-[88px] h-[88px] rounded-lg overflow-hidden shrink-0 bg-[#111e35]">
                    <img
                      src={getImage(task)}
                  ...
                  <div className="flex-1 flex flex-col justify-between min-w-0">
                    <div>
                      ...
                      <div className="flex items-center text-xs text-white/40 mt-1 gap-1">
                        <Home className="w-3 h-3 shrink-0" />
                        <span className="truncate">{task.propertyType}</span>
                      </div>
                      <div className="flex items-center text-xs text-white/40 mt-0.5 gap-1">
