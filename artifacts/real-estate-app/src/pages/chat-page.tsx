...
import {
  ...
  Volume2, AlertCircle, ChevronDown,
} from "lucide-react";
...
const PHONE_RE_FRONTEND = /(\+?\d[\d\s\-().]{6,}\d)/;
const URL_RE_FRONTEND = /(https?:\/\/|www\.|[a-zA-Z0-9-]+\.(com|net|org|io|app|co|ng|uk|us|me|info|biz|site|online|shop|store|xyz|gg|tv|fm)\b)/i;

function containsForbidden(text: string): boolean {
  return PHONE_RE_FRONTEND.test(text) || URL_RE_FRONTEND.test(text);
}

function ChatDrawer({
  peer, myId, msgs, onClose, onSend, onCall, callingEnabled, chatError, onClearChatError,
}: {
  ...
  function handleSend() {
    ...
    if (t && containsForbidden(t)) {
      setSendError("Phone numbers and links are not allowed in chat.");
      return;
    }
    setSendError(null);
    onClearChatError();
  ...
  return (
    ...
    >
      ...
      {(sendError || chatError) && (
        <div className="px-4 py-2 bg-red-50 border-t border-red-100 text-xs text-red-600 font-medium flex items-center justify-between gap-2 shrink-0">
          <span>{sendError || chatError}</span>
          <button onClick={() => { setSendError(null); onClearChatError(); }} className="text-red-400 hover:text-red-600 shrink-0">
      ...
      <div className="flex items-end gap-2 px-3 py-3 border-t bg-white shrink-0">
        ...
        <textarea value={text} onChange={(e) => { setText(e.target.value); if (sendError) setSendError(null); }} onKeyDown={handleKey}
...
export default function ChatPage() {
  ...
  const { user } = useAuth();
  const { onlineUsers, messages, connected, banned, banError, chatError, clearChatError, settings, onCallSignal, sendMessage, sendSignal, loadHistory } = useChat();
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col" style={{ maxWidth: 430, margin: "0 auto" }}>
      ...
      <div className="flex-1 px-4 py-4">
        ...
        {!settings.chatEnabled ? (
          ...
          const offlineUsers = allUsers.filter((u) => !onlineSet.has(u.id));
          const onlineUsersList = allUsers.filter((u) => onlineSet.has(u.id));

          function MemberRow({ u, isOnline }: { u: AllUser; isOnline: boolean }) {
            const name = `${u.firstName} ${u.surname}`.trim();
            const unreadCount = (messages[u.id] ?? []).filter((m) => m.senderId === u.id).length;
            return (
              <div
                className="flex items-center gap-2.5 px-3 py-2 bg-white rounded-xl border border-gray-100 shadow-sm active:scale-[0.98] transition-transform cursor-pointer"
                onClick={() => openChat(u)}
              >
                <div className="relative shrink-0">
                  <Avatar name={name} avatar={u.avatar} size={32} />
                  <div className={`absolute bottom-0 right-0 w-2 h-2 rounded-full border-[1.5px] border-white ${isOnline ? "bg-green-400" : "bg-slate-300"}`} />
                  {unreadCount > 0 && (
                    <span className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-amber-500 text-white text-[8px] font-bold flex items-center justify-center leading-none">{unreadCount}</span>
                  )}
                </div>
                <p className="flex-1 min-w-0 font-medium text-slate-800 text-xs truncate">{name}</p>
                <div className="flex items-center gap-1 shrink-0">
                  {settings.callingEnabled && isOnline && (
                    <button
                      onClick={(e) => { e.stopPropagation(); startCall(u); }}
                      disabled={callState !== "idle"}
                      className="w-6 h-6 rounded-full flex items-center justify-center bg-amber-50 text-amber-600 hover:bg-amber-100 active:scale-90 transition-all disabled:opacity-40"
                    >
                      <Phone className="w-3 h-3" />
                    </button>
                  )}
                  <button
                    onClick={(e) => { e.stopPropagation(); openChat(u); }}
                    className="w-6 h-6 rounded-full flex items-center justify-center bg-slate-50 text-slate-400 hover:bg-slate-100 active:scale-90 transition-all"
                  >
          ...
          function CollapsibleSection({
            label, count, accentClass, dotClass, children,
          }: {
            label: string; count: number; accentClass: string; dotClass: string; children: React.ReactNode;
          }) {
              ...
              <div className="rounded-xl border border-gray-100 overflow-hidden bg-white shadow-sm">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs font-semibold uppercase tracking-wide ${accentClass}`}>{label}</span>
                    <span className="text-[10px] font-semibold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded-full">{count}</span>
                  </div>
                  <ChevronDown className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-200 ${open ? "rotate-180" : ""}`} />
                </button>
                {open && (
                  <div className="px-2 pb-2 flex flex-col gap-1.5 border-t border-gray-50">
                    {count === 0 ? (
                      <p className="text-[11px] text-slate-300 italic py-2 text-center">None</p>
                    ) : children}
                  </div>
                )}
              </div>
          ...
          return (
            <div className="flex flex-col gap-2">
              <CollapsibleSection label="Online" count={onlineUsersList.length} accentClass="text-green-600" dotClass="bg-green-400">
                {onlineUsersList.map((u) => <MemberRow key={u.id} u={u} isOnline={true} />)}
              </CollapsibleSection>
              <CollapsibleSection label="Offline" count={offlineUsers.length} accentClass="text-slate-500" dotClass="bg-slate-300">
                {offlineUsers.map((u) => <MemberRow key={u.id} u={u} isOnline={false} />)}
              </CollapsibleSection>
            </div>
          );
        })()}
