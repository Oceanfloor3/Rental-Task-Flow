    ...
    <div className="min-h-screen bg-slate-50 flex flex-col" style={{ maxWidth: 430, margin: "0 auto" }}>
      ...
      <div className="flex-1 px-4 py-4">
        ...
        {!settings.chatEnabled ? (
          ...
          function MemberCard({ u, isOnline }: { u: AllUser; isOnline: boolean }) {
            ...
            return (
              <motion.div key={u.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                className="flex flex-col items-center gap-1.5 bg-white rounded-2xl p-3 shadow-sm border border-gray-100 active:scale-[0.97] transition-transform cursor-pointer"
                onClick={() => openChat(u)}>
                <div className="relative">
                  <Avatar name={name} avatar={u.avatar} size={40} />
                  <div className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-white ${isOnline ? "bg-green-400" : "bg-slate-300"}`} />
                  {unreadCount > 0 && (
                    <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-amber-500 text-white text-[9px] font-bold flex items-center justify-center">{unreadCount}</span>
                  )}
                </div>
                <p className="font-semibold text-slate-800 text-xs text-center truncate w-full leading-tight">{name}</p>
                <div className="flex items-center gap-1.5">
                  {settings.callingEnabled && isOnline && (
                    <button
                      onClick={(e) => { e.stopPropagation(); startCall(u); }}
                      disabled={callState !== "idle"}
                      className="w-7 h-7 rounded-full flex items-center justify-center bg-amber-50 text-amber-600 hover:bg-amber-100 active:scale-90 transition-all disabled:opacity-40"
                    >
                  ...
                  <button
                    ...
                    className="w-7 h-7 rounded-full flex items-center justify-center bg-slate-50 text-slate-400 hover:bg-slate-100 active:scale-90 transition-all"
                  >
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex flex-col gap-2">
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">
                  Offline {offlineUsers.length > 0 && <span className="text-slate-300">({offlineUsers.length})</span>}
                </p>
                {offlineUsers.length === 0 ? (
                  <p className="text-xs text-slate-300 italic">None</p>
                ) : (
                  offlineUsers.map((u) => <MemberCard key={u.id} u={u} isOnline={false} />)
                )}
              </div>
              {/* Right column — Online */}
              <div className="flex flex-col gap-2">
                <p className="text-xs font-semibold text-green-500 uppercase tracking-wide">
                  Online {onlineUsersList.length > 0 && <span className="text-green-300">({onlineUsersList.length})</span>}
                </p>
                {onlineUsersList.length === 0 ? (
                  <p className="text-xs text-slate-300 italic">None online</p>
                ) : (
                  onlineUsersList.map((u) => <MemberCard key={u.id} u={u} isOnline={true} />)
                )}
              </div>
            </div>
          );
