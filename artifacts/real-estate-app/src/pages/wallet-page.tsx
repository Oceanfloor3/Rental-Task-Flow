          ...
          {keys.map((k, i) => (
            <button
              ...
              className={`h-14 rounded-2xl text-lg font-bold transition-all active:scale-95 ${
                ...
                k === "⌫" ? "bg-[#132840] text-slate-600 hover:bg-[#2a5585] active:bg-gray-300" :
                "bg-[#132840] border border-white/40 text-slate-800 hover:bg-[#2a5585] active:bg-[#25537f]"
...
function WithdrawPage({ profile, onBack }: { profile: any; onBack: () => void }) {
  ...
  return (
    <motion.div
      ...
      transition={{ type: "spring", damping: 28, stiffness: 280 }}
      className="min-h-screen flex flex-col bg-gradient-to-b from-[#132840] to-[#0f2240]"
    >
      {/* Header */}
      ...
      <div className="flex-1 px-4 pb-10 space-y-5 overflow-y-auto">
        ...
        {/* Bank info card */}
        <div className="bg-white rounded-3xl shadow-sm border border-white/40 p-5 space-y-4">
          <div className="flex items-center gap-2 pb-1 border-b border-white/40">
            <div className="w-8 h-8 bg-[#2a5585] rounded-xl flex items-center justify-center">
              <ArrowDownLeft className="w-4 h-4 text-[#9a7a18]" />
            </div>
            <div>
              <p className="text-sm font-bold text-slate-800">Bank Account</p>
              <p className="text-xs text-white/80">Funds will be sent to this account</p>
            </div>
          </div>
          <div className="bg-[#132840] rounded-2xl p-4 border border-white/40 space-y-2">
                  ...
                  <button
                    ...
                    className={`rounded-xl py-3 px-3 text-sm font-bold border-2 transition-all ${
                      isSelected
                        ...
                        : canAfford
                          ? "bg-white border-white/35 text-slate-700 hover:border-[#c9a020]/60 hover:bg-[#132840]"
          ...
          {selected && selected > 0 && (
            <motion.div
              ...
              className="bg-[#132840] rounded-2xl p-4 space-y-1.5 border border-white/40"
            >
              ...
              <div className="border-t border-white/35 pt-1.5 flex justify-between text-xs">
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 pt-5 pb-3 shrink-0">
        ...
        <div>
          ...
          {limit !== null && (
            <p className="text-xs text-white/90 mt-0.5 ml-7">
      ...
      <div className="flex-1 px-4 pb-10 space-y-5 overflow-y-auto">
        ...
        {/* Weekly limit banner */}
        {limit !== null && (
          <div className={`rounded-2xl px-4 py-3 flex items-center justify-between border ${remaining === 0 ? "bg-red-50 border-red-100" : "bg-[#132840] border-white/40"}`}>
            <div>
              <p className={`text-xs font-bold ${remaining === 0 ? "text-red-600" : "text-[#9a7a18]"}`}>Weekly Transfer Allowance</p>
              <p className="text-xs text-white/80 mt-0.5">Resets every Monday</p>
            </div>
            <div className="text-right">
              <p className={`text-sm font-black ${remaining === 0 ? "text-red-600" : "text-slate-800"}`}>
                ₦{(remaining ?? 0).toLocaleString("en-NG")} left
              </p>
              <p className="text-xs text-white/80">of ₦{limit.toLocaleString("en-NG")}</p>
            </div>
          </div>
        )}

        {/* Transfer form card */}
        <div className="bg-white rounded-3xl shadow-sm border border-white/40 p-5 space-y-5">
          <div className="flex items-center gap-2 pb-1 border-b border-white/40">
            <div className="w-8 h-8 bg-[#2a5585] rounded-xl flex items-center justify-center">
            ...
            <div>
              ...
              <p className="text-xs text-white/80">Instant transfer, no fees</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              />
              <p className="text-xs text-white/80">Enter their referral code, username, or email address</p>
            </div>
...
const txConfig: Record<string, { label: string; icon: React.ReactNode; color: string; bg: string; sign: "+" | "-" }> = {
  ...
  withdrawal:             { label: "Withdrawal", icon: <ArrowDownLeft className="w-4 h-4 text-[#9a7a18]" />, color: "text-[#9a7a18]", bg: "bg-[#2a5585]", sign: "-" },
  withdrawal_requested:   { label: "Withdrawal", icon: <ArrowDownLeft className="w-4 h-4 text-[#9a7a18]" />, color: "text-[#9a7a18]", bg: "bg-[#2a5585]", sign: "-" },
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-[#132840] to-[#0f2240]">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-8 space-y-4">
        ...
        <div className="bg-gradient-to-r from-[#c9a020] to-[#9a7a18] rounded-2xl p-5 text-white shadow-lg">
          ...
          <div className="mt-4 flex gap-2">
            <button
              ...
              className={`flex-1 py-2.5 rounded-xl text-sm font-semibold border border-white/40 flex items-center justify-center gap-2 transition-all ${
            ...
            <button
              ...
              className="flex-1 py-2.5 rounded-xl text-sm font-semibold border border-white/40 bg-white/20 hover:bg-white/30 active:scale-95 transition-all flex items-center justify-center gap-2"
            >
              <Send className="w-4 h-4" />
              Transfer
            </button>
        ...
        <div className="flex items-center justify-between pt-1">
          ...
          <button onClick={() => refetchTx()} className="p-1.5 rounded-lg bg-white/15 hover:bg-white border border-white/35 transition-all active:scale-90">
            <RefreshCw className="w-3.5 h-3.5 text-slate-400" />
          </button>
        </div>

        {/* Tabs */}
        <div className="flex gap-2 bg-white/15 backdrop-blur-sm rounded-xl p-1 border border-white/40">
            ...
            <div className="space-y-2">
              {allTxns.map((t: any) => {
                const cfg = txConfig[t.type] ?? {
                  label: t.type,
                  icon: <History className="w-4 h-4 text-white/80" />,
                ...
                return (
                  <div key={t.id} className="bg-white rounded-2xl border border-white/40 px-4 py-3.5 shadow-sm flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {isWithdrawal && (
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${txStatusStyle[txStatus] ?? "bg-[#132840] text-white/90"}`}>
                            {txStatus === "completed" ? "Completed" : txStatus.charAt(0).toUpperCase() + txStatus.slice(1)}
                          </span>
                        )}
                      </div>
                      <p className="text-xs text-white/80 truncate">{t.description || t.relatedUserName || "—"}</p>
                      <p className="text-[10px] text-white/68 mt-0.5">
          ...
          withdrawalTxns.length === 0 ? (
            ...
            <div className="space-y-3">
              {withdrawalTxns.map((t: any) => (
                <div key={t.id} className="bg-white rounded-2xl border border-white/40 p-4 shadow-sm space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-[#2a5585] rounded-full flex items-center justify-center shrink-0">
                        <ArrowDownLeft className="w-4 h-4 text-[#9a7a18]" />
                      </div>
                      <div>
                    ...
                    <div className="text-right">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${withdrawalStatusStyle[t.status] ?? "bg-[#132840] text-white/90"}`}>
                        {t.status === "approved" ? "Completed" : t.status.charAt(0).toUpperCase() + t.status.slice(1)}
                      </span>
                    </div>
                  </div>
                  <div className="border-t border-white/40 pt-2.5 space-y-1.5">
                    <div className="flex justify-between text-xs">
